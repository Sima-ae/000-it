...
export function NewsPagination({
  ...
  const btnNext =
    "inline-flex min-h-10 min-w-10 items-center justify-center rounded-md border border-brand bg-brand px-3 text-sm font-medium text-white transition hover:bg-brand/90 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand/40";
  const btnDisabled =
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-wrap items-center justify-center gap-2">
        ...
        {next ? (
          <SoftLink href={hrefFor(next)} className={btnNext} aria-label={labels.next}>
            {labels.next}
          </SoftLink>
        ) : (
          <span className={btnDisabled} aria-disabled="true">
